function [velocity,bed,moving]=transport103(velocity, bed, moving,i);
...
%51 Franklin Street, Fifth Floor, Boston, MA 02110-1301 USA.
    ...
    %This is all for W=3cm/sec which is for about 300 micron sand
    [indn_alldown,indp_alldown]=find(abs(velocity(:,:))<=60);  %high settling rates
    len_alldown=length(indn_alldown);
    [indn_onedown,indp_onedown]=find(abs(velocity(:,:))>60 & abs(velocity(:,:))<=80); %settling
    len_onedown=length(indn_onedown);
    [indn_oneup,indp_oneup]=find(abs(velocity(:,:))>80 & abs(velocity(:,:))<=90); %erosion 1
    len_oneup=length(indn_oneup);
    [indn_twoup,indp_twoup]=find(abs(velocity(:,:))>90 & abs(velocity(:,:))<=110); %erosion 2
    len_twoup=length(indn_twoup);
    [indn_3up, indp_3up]=find(abs(velocity(:,:))>110);  %erosion 3
